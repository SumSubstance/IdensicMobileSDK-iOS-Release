...
//  IdensicMobileSDK
//
//  Copyright © 2026 Sumsub. All rights reserved.
//
import UIKit
        ...
        sdk.onEvent { (sdk, event) in
            ...
            case .applicantLoaded:
                if let event = event as? SNSEventApplicantLoaded {
                    print("onEvent: Applicant [\(event.applicantId)] has been loaded" + (event.actionId.map { "(action: \($0))" } ?? ""))
                }
            ...
            case .stepCompleted:
                if let event = event as? SNSEventStepCompleted {
                    print("onEvent: Step [\(event.idDocSetType)] has been \(event.isCancelled ? "cancelled" : "fulfilled") at level: \(String(describing: event.levelName))")
                }
